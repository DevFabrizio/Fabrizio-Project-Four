...
![This is the Entity-Relationship Diagram for the project](https://res.cloudinary.com/devfabrizio/image/upload/v1691304643/Project%20Matinicus/PP4_ERD_ewxjif.png)

In this ERD the specific model for the user is provided by the allauth library for django. That is the reason why the model is not present in the models.py file. The two models are linked by the user id, which is applied to the reservation model when the user who makes a booking is logged in on the webpage. The type of fields which I decided to use are slightly more than what a normal reservation would need but the more complete set of information provided gives the kitchen a better start point on how to manage eventual food related issue and the waiting personnel a chance to prepare for the presence of children.